...
class TestInstanceNameFlagTrue(cloudstackTestCase):
    ...
    def test_01_custom_hostname_instancename_true(self):
        """ Verify custom hostname for the instance
        """

        # Validate the following
        # 1. Set the vm.instancename.flog to true. Hostname and displayname
        ...
        self.debug("vm.displayname: %s, origional: %s" %
                        (vm.displayname,
    ...
    @attr(configuration='vm.instancename.flag')
    @attr(tags=["advanced", "basic", "sg", "eip", "advancedns", "simulator"])
    def test_02_custom_hostname_instancename_true(self):
        """ Verify custom hostname for the instance
        """
    ...
    def test_03_duplicate_name(self):
        ...
        # Readding display name property
        ...
        self.assertEqual(
                        ...
                        True,
                        "List configs shall return a valid list"
                        )

        config = configs[0]
        self.assertEqual(
                        str(config.value).lower(),
                        str(True).lower(),
                        "vm.instancename.flag should be set to true"
                        )
...
class TestInstanceNameFlagFalse(cloudstackTestCase):
    ...
    def test_01_custom_hostname_instancename_false(self):
        ...
        self.assertEqual(
                        str(config.value).lower(),
                        str(False).lower(),
                        "vm.instancename.flag should be set to false"
                        )
        ...
        #internal Name = i-<user ID>-<VM ID>- Instance_name
        internal_name = "i-" + str(account_id) + "-" + str(vmid) + "-VM"
        self.debug("Internal name: %s" % internal_name)
        self.assertEqual(
                        vm.instancename,
                        internal_name,
